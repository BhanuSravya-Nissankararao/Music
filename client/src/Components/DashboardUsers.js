...
export default function DashboardUsers() {
  ...
  return (
    <div className='w-full p-4 flex items-center justify-center flex-col'>
      ...
      {/* tabular data */}
      <div className='relative w-full py-12 min-h-[400px] overflow-x-scroll my-4 flex flex-col items-center justify-start p-4 
      border border-gray-300 rounded-md gap-3'>
        ...
        <div className='w-full min-w-[750px] flex items-center justify-between'>
          ...
          <p className='text-sm text-textColor font-semibold w-275 min-w-[160px] text-center'>E-Mail</p>
...
export const DashboardUserCard = ({data,index})=>{
  ...
  return(
    ...
    className="relative w-full rounded-md flex items-center justify-between py-4 bg-lightOverlay cursor-pointer
    hover:bg-card hover:shadow-md">
      ...
      <div className='w-275 min-w-[160px] text-center flex items-center justify-center gap-6 relative'>
        ...
        }
        {
          ...
          <p className='text-textcolor text-[12] font-semibold'>Are You Sure, do you want the user as{" "}
          <span>{data.role === "admin" ? "Member" : "Admin"}</span> ?
          </p>
          <div className='flex items-center gap-4'>
            <motion.button whileTap={{scale:0.75}} className='outline-none border-none text-sm px-4 py-1 rounded-md bg-blue-200
            text-black hover:shadow-md'
            onClick={()=>updateUserRole (data._id,data.role === "admin" ? "member" : "admin")}>
              Yes
            </motion.button>
            <motion.button whileTap={{scale:0.75}} className='outline-none border-none text-sm px-4 py-1 rounded-md bg-gray-200
            text-black hover:shadow-md' onClick={()=> setisUserRoleUpdated(false)}>
              No
            </motion.button>
          </div>
        </motion.div>
